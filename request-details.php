...
try {
    ...
    SELECT 
        ...
        req.id as requester_id, req.fullnames, req.email, req.phone_number, req.requester_type,
...
");
    ...
    if (strtolower($request['status']) === 'rejected') {
        ...
        if (isset($request['rejected_at']) && strtolower($request['rejected_at']) === 'reviewer') {
            $rejectionReason = $request['review_comments'] ?? 'No reason provided';
        } elseif (isset($request['rejected_at']) && strtolower($request['rejected_at']) === 'approver') {
            $rejectionReason = $request['approver_comments'] ?? 'No reason provided';
        } else {
            // Fallback if rejected_at isn't set properly
            $rejectionReason = $request['review_comments'] ?? $request['approver_comments'] ?? 'No rejection reason available';
        }
    }
    ...
    if (isset($request['requester_id'])) {
        ...
        SELECT document_name, document_file_path 
        FROM requestors_documents 
        WHERE request_id = ? AND requester_id = ?
    ");
        $docStmt->bind_param("ii", $requestId, $request['requester_id']);
        $docStmt->execute();
    ...
    <body>
        ...
        <div class="detail-container">
            ...
            <?php if (strtolower($request['status']) === 'rejected' && !empty($rejectionReason)): ?>
                <div class="detail-section">
                    ...
                    <div class="detail-row">
                        ...
                        <div class="detail-value">
                            <div style="background: #fff; padding: 1rem; border-radius: var(--border-radius); border: 1px solid #eee;">
            ...
            <?php if (!empty($documents)): ?>
                <div class="detail-section">
                    <h3><i class="bi bi-file-earmark"></i> Uploaded Documents</h3>
                    <div class="documents-grid">
                        <?php foreach ($documents as $document): ?>
                            <div class="document-card">
                                <div class="document-icon">
                                    <?php
                                    $extension = pathinfo($document['document_file_path'], PATHINFO_EXTENSION);
                                ...
                                <div class="document-info">
                                    ...
                                    <div class="document-actions">
                                        <a href="<?php echo htmlspecialchars($document['document_file_path']); ?>"
                                            target="_blank"
